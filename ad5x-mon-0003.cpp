...
// Safely get active IP across cores (Guarantees non-empty IP if any printer exists)
String getActiveIp() {
  String ip = "";
...
bool sendPrinterTcpCommands(const std::vector<String>& commands, int delayBetweenMs = 40) {
...
bool sendPrinterTcpCommand(const char* cmd) {
  ...
  return sendPrinterTcpCommands(cmds, 40);
}
...
"function findPrinter(){var b=document.getElementById('btnFind');b.innerText='✨ Flashing...';b.disabled=true;sendAction('flash_lights').then(function(){setTimeout(function(){b.innerText='💡 Flash Lights';b.disabled=false;fetchData();},1200);});}"
...
void setup() {
  ...
  // Printer Management Endpoints (using HTTP_ANY to catch GET/POST without dropouts)
  ...
  server.on("/api/cmd", HTTP_ANY, [](){
    if (server.hasArg("action")) {
      ...
      if (act == "toggle_light") {
        ...
        std::vector<String> flashCmds = {"~M146\r\n", "~M146\r\n", "~M146\r\n", "~M146\r\n"};
        ok = sendPrinterTcpCommands(flashCmds, 150);
      }
        ...
        ok = sendPrinterTcpCommands(cmds, 30);
      }
