...
// --- Network Setup ---
// Set to true to use a fixed IP, or false to use automatic DHCP from your router
...
struct PrinterData {
  String status = "No Printer Added";
...
// --- Send Control Commands via Port 8899 ---
bool sendPrinterTcpCommand(const char* cmd) {
  ...
  if (!client.connect(targetIp.c_str(), printer_tcp_port, 2000)) {
    Serial.println("[TCP] Command connection failed");
    return false;
  }
...
void updatePrinterTelemetry() {
  ...
  if (tIndex != -1) {
    nozzle = tempResp.substring(tIndex + 3, tempResp.indexOf(" ", tIndex)).toFloat();
  }
  int bIndex = tempResp.indexOf("B:");
  if (bIndex != -1) {
    bed = tempResp.substring(bIndex + 2, tempResp.indexOf(" ", bIndex)).toFloat();
  }
...
// --- Dashboard UI Template (Flash PROGMEM) ---
...
"var p=document.getElementById('btnPause');if(d.status==='Paused'){p.innerText='Resume Print';}else{p.innerText='Pause Print';}}).catch(function(){document.getElementById('status').innerText='Offline';});}"
  ...
  server.on("/api/cmd", HTTP_POST, [](){
    if (server.hasArg("action")) {
      ...
      if (act == "toggle_light") {
        ...
        // Flash chamber LEDs 4 times as a visual beacon
        for (int i = 0; i < 4; i++) {
      ...
      else if (act == "cooldown") ok = sendPrinterTcpCommand("~M104 S0\r\n~M140 S0\r\n");
      else if (act == "preheat_pla") ok = sendPrinterTcpCommand("~M104 S210\r\n~M140 S60\r\n");
      else if (act == "preheat_petg") ok = sendPrinterTcpCommand("~M104 S240\r\n~M140 S80\r\n");
      else if (act == "bed_soak") ok = sendPrinterTcpCommand("~M140 S60\r\n");
      else if (act == "home") ok = sendPrinterTcpCommand("~G28\r\n");
      else if (act == "lower_bed") ok = sendPrinterTcpCommand("~G91\r\n~G1 Z50 F1200\r\n~G90\r\n");
      else if (act == "center_head") ok = sendPrinterTcpCommand("~G90\r\n~G1 X110 Y110 Z50 F3600\r\n");
      else if (act == "motors_off") ok = sendPrinterTcpCommand("~M84\r\n");
      else if (act == "toggle_fan") {
        if (xSemaphoreTake(printerMutex, pdMS_TO_TICKS(100))) {
          printer.part_fan = !printer.part_fan;
          bool state = printer.part_fan;
          xSemaphoreGive(printerMutex);
          ok = sendPrinterTcpCommand(state ? "~M106 P1 S255\r\n" : "~M107 P1\r\n");
        }
      }
      else if (act == "toggle_chamber_fan") {
        if (xSemaphoreTake(printerMutex, pdMS_TO_TICKS(100))) {
          printer.chamber_fan = !printer.chamber_fan;
          bool state = printer.chamber_fan;
          xSemaphoreGive(printerMutex);
          ok = sendPrinterTcpCommand(state ? "~M106 P2 S255\r\n" : "~M107 P2\r\n");
        }
      }
