...
// --- Network Setup (false = DHCP, true = Static IP) ---
...
struct PrinterData {
  String status = "No Printer Configured";
...
void loadPrintersFromNVS() {
  ...
  // Sanitized start: unconfigured if no printers are stored
...
void updatePrinterTelemetry() {
  ...
  if (targetIp.length() == 0) {
    if (xSemaphoreTake(printerMutex, pdMS_TO_TICKS(1000))) {
      ...
      printer.nozzle_target = 0.0;
      printer.bed_temp = 0.0;
      printer.bed_target = 0.0;
      printer.progress = 0;
      xSemaphoreGive(printerMutex);
    }
    return;
  ...
  if (!xSemaphoreTake(tcpMutex, pdMS_TO_TICKS(1500))) {
    return;
  }
  ...
  if (tIndex != -1) {
    ...
    if (slash != -1 && (space == -1 || slash < space)) {
      ...
      if (nextSpace != -1) {
        nozzle_tgt = tempResp.substring(slash + 1, nextSpace).toFloat();
      } else {
        nozzle_tgt = tempResp.substring(slash + 1).toFloat();
      }
    } else if (space != -1) {
  ...
  if (bIndex != -1) {
    ...
    if (slash != -1 && (space == -1 || slash < space)) {
      ...
      int nextSpace = tempResp.indexOf(" ", slash);
      if (nextSpace != -1) {
        bed_tgt = tempResp.substring(slash + 1, nextSpace).toFloat();
      } else {
        bed_tgt = tempResp.substring(slash + 1).toFloat();
      }
    } else if (space != -1) {
...
":root{--bg-body:#0f172a;--bg-card:#1e293b;--btn-bg:#334155;--btn-hover:#475569;--btn-warn:#b45309;--btn-warn-hover:#d97706;--btn-danger:#dc2626;--btn-danger-hover:#ef4444;--btn-success:#16a34a;--btn-blue:#2563eb;--btn-blue-hover:#1d4ed8;--text-main:#f8fafc;--text-muted:#94a3b8;--border-color:#334155;}"
...
".btn-preheat-active{background-color:var(--btn-danger) !important;color:#ffffff !important;box-shadow:0 0 10px rgba(220,38,38,0.7) !important;}"
".btn-motion-active{background-color:#d97706 !important;color:#ffffff !important;box-shadow:0 0 10px rgba(217,119,6,0.7) !important;}"
...
"<button id='btnCool' class='blue' onclick='triggerPreheat(\"cooldown\")'>Cool Off</button>"
...
"<div class='btn-grid-4'>"
"<button onclick='triggerMotion(\"home\", this)'>Home (G28)</button>"
"<button onclick='triggerMotion(\"lower_bed\", this)'>Lower Bed</button>"
...
"<div class='row'><span class='label'>LED:</span><span id='light' class='val'>--</span></div>"
...
"function deletePrinter(idx){if(confirm('Delete this printer?')){fetch('/api/printers/delete?idx='+idx,{method:'POST'}).then(function(){loadPrinters(true);setTimeout(fetchData,600);});}}"
...
"function sendAction(act){return fetch('/api/cmd?action='+act,{method:'POST'}).then(function(){setTimeout(fetchData,600);});}"
...
"function findPrinter(){var b=document.getElementById('btnFind');b.innerText='✨ Flashing...';b.disabled=true;sendAction('flash_lights').then(function(){setTimeout(function(){b.innerText='💡 Flash Lights';b.disabled=false;fetchData();},2600);});}"
